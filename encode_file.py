...
def encode_IFTSAI_file(file):
    ...
    for data in data_list:
        ...
        for element in data:
            line += element
        message_code = get_element(get_one_line(line, "UNH"), 1, "+")
        message_date = get_element(get_one_line(line, "DTM+137"), 1, "+")
        message_date = string_to_date(message_date)
        route_name = get_element(get_one_line(line, "FTX"), 4, "+")

        voyage_name = get_element(get_one_line(line, "TDT"), 8, "+")
        voyage_name = get_element(voyage_name, 3, ":")

        voyage_no = get_element(get_one_line(line, "TDT"), 2, "+")
        place_of_receipt = get_element(get_one_line(line, "LOC+88"), 2, "+")[0:5]
        place_of_receipt_date = get_element(get_one_line(line, "DTM+180"), 1, "+")
        place_of_receipt_date = string_to_date(place_of_receipt_date)
        place_of_loading = get_element(get_one_line(line, "LOC+9"), 2, "+")[0:5]
        place_of_loading_date = get_element(get_one_line(line, "DTM+133"), 2, "+")
        place_of_loading_date = string_to_date(place_of_loading_date)
        place_of_discharge = get_element(get_one_line(line, "LOC+11"), 2, "+")[0:5]
        place_of_discharge_date = get_element(get_one_line(line, "DTM+132"), 2, "+")
        place_of_discharge_date = string_to_date(place_of_discharge_date)
        place_of_delivery = get_element(get_one_line(line, "LOC+7"), 2, "+")[0:5]
        place_of_delivery_date = get_element(get_one_line(line, "DTM+17"), 2, "+")
